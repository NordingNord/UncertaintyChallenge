...
class Classifier(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        outputs = []
        backbone_x = self.embed(x) # Backbone
        for _ in range(5): # Magic number iterations
            dropout_x = self.dropout(backbone_x) # Added dropout
            outputs.append(self.head(dropout_x).unsqueeze(0))
        # Get mean
